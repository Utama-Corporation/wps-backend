...
const templatePath = path.join(__dirname, "bj-label-pdf.html");
...
/**
 * @param {object} data hasil labelBjService.getLabelData(noBJ) + { qrBase64 }
 * @returns {string} HTML siap render ke PDF
 */
function buildBjLabelHtml(data) {
  ...
  return html
    .replace(/{{noBJ}}/g, esc(data.noBJ))
    .replace("{{jenisKayu}}", esc(data.jenisKayu))
    .replace("{{namaBJ}}", esc(data.namaBJ))
...
module.exports = { buildBjLabelHtml };
