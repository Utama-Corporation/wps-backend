...
const templatePath = path.join(__dirname, "lmt-label-pdf.html");
...
/**
 * @param {object} data hasil labelLmtService.getLabelData(noLMT) + { qrBase64 }
 * @returns {string} HTML siap render ke PDF
 */
function buildLmtLabelHtml(data) {
  ...
  return html
    .replace(/{{noLaminating}}/g, esc(data.noLaminating))
...
module.exports = { buildLmtLabelHtml };
