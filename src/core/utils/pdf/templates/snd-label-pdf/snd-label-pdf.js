...
const templatePath = path.join(__dirname, "snd-label-pdf.html");
...
/**
 * @param {object} data hasil labelSndService.getLabelData(noSND) + { qrBase64 }
 * @returns {string} HTML siap render ke PDF
 */
function buildSndLabelHtml(data) {
  ...
  return html
    .replace(/{{noSanding}}/g, esc(data.noSanding))
...
module.exports = { buildSndLabelHtml };
