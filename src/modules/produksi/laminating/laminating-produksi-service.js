...
async function getHistory() {
  ...
  const outputs = await req.query(`
    ...
    FROM LaminatingProduksiOutput O
    INNER JOIN Laminating_h H ON H.NoLaminating = O.NoLaminating
...
async function getNextNoProduksi() {
  const pool = await poolPromise;
  const result = await pool.request().query(`
    SELECT 'UA.' + FORMAT(RIGHT(ISNULL(MAX(NoProduksi), 'UA.000000'), 6) + 1, '000000') AS NoProduksi
    FROM LaminatingProduksi_h
  `);
  return result.recordset[0] ? result.recordset[0].NoProduksi : "UA.000001";
}

async function getNextNoLabel() {
  const pool = await poolPromise;
  const result = await pool.request().query(`
    SELECT ISNULL('U.' + FORMAT(RIGHT(MAX(NoLaminating), 6) + 1, '000000'), 'U.000001') AS NoLaminating
    FROM Laminating_h
  `);
  return result.recordset[0] ? result.recordset[0].NoLaminating : "U.000001";
}
...
async function getMasterOptions() {
  ...
  const [jenisKayu, grade, spk] = await Promise.all([
    ...
    pool.request().query(`
      ...
      WHERE A.[Enable] = 1 AND B.Category = 'Laminating'
...
async function saveHeader({
  ...
  const result = await req.query(`
    ...
    SELECT @np = 'UA.' + FORMAT(RIGHT(ISNULL(MAX(NoProduksi), 'UA.000000'), 6) + 1, '000000')
    FROM LaminatingProduksi_h;
    INSERT INTO LaminatingProduksi_h (NoProduksi, [Shift], Tanggal, IdMesin, IdOperator, JamKerja, JmlhAnggota, HourMeter, JamLembur)
...
async function updateHeader({
  ...
  await req.query(`
    UPDATE LaminatingProduksi_h SET
      Shift = @sh,
      JmlhAnggota = @ja,
      JamKerja = @jk,
      JamLembur = @jl,
...
async function createLabel({
  ...
  return result.recordset[0] ? result.recordset[0].NoLaminating : null;
}

const INPUT_TABLES = {
  BARANGJADI: "LaminatingProduksiInputBarangJadi",
  CCAKHIR: "LaminatingProduksiInputCCAkhir",
  MOULDING: "LaminatingProduksiInputMoulding",
  SANDING: "LaminatingProduksiInputSanding",
};
...
async function addOutput({
  noProduksi, noLaminating, idJenisKayu, idGrade, noSPKAsal, noSPK,
  tebal, lebar, panjang, jmlhBatang,
}) {
  const pool = await poolPromise;
  const req = pool.request();
  req.input("np", sql.VarChar(20), noProduksi);
  req.input("nl", sql.VarChar(20), noLaminating);
  req.input("jk", sql.Int, idJenisKayu);
  req.input("gr", sql.Int, idGrade);
  req.input("stasal", sql.VarChar(50), noSPKAsal || "");
  req.input("nospk", sql.VarChar(50), noSPK || "");
  ...
  await req.query(`
    IF NOT EXISTS (SELECT 1 FROM Laminating_h WHERE NoLaminating = @nl)
    BEGIN
      INSERT INTO Laminating_h (NoLaminating, IdJenisKayu, IdGrade, IdOrgTelly, DateCreate, DateUsage, IdUOMTblLebar, IdUOMPanjang, NoSPK, Jam, IsReject, NoMouldingAsal, IdWarehouse, IdFJProfile, IdLokasi, IsLembur, IdFisik, NoSPKAsal)
      VALUES (@nl, @jk, @gr, 1, GETDATE(), NULL, 1, 1, NULLIF(@nospk, ''), FORMAT(GETDATE(), 'HH:mm'), 0, NULL, 7, NULL, NULL, 0, 5, NULLIF(@stasal, ''));
      INSERT INTO Laminating_d (NoLaminating, NoUrut, Tebal, Lebar, Panjang, JmlhBatang)
      VALUES (@nl, 1, @tb, @lb, @pj, @bt);
    END
    INSERT INTO LaminatingProduksiOutput (NoProduksi, NoLaminating)
    VALUES (@np, @nl);
  `);

  return { noProduksi, noLaminating };
}

async function removeOutput({ noProduksi, noLaminating }) {
  const pool = await poolPromise;
  const req = pool.request();
  req.input("np", sql.VarChar(20), noProduksi);
  req.input("nl", sql.VarChar(20), noLaminating);
  ...
  await req.query(`
    DELETE FROM LaminatingProduksiOutput WHERE NoProduksi = @np AND NoLaminating = @nl
  `);
