...
async function getStock(tgl) {
  ...
  const result = await req.query(`
    SELECT
      (CASE WHEN A.IdFisik IS NULL THEN G.Singkatan ELSE H.Singkatan END + ' ' + C.Jenis + ' ' + I.NamaGrade) AS Jenis,
    ...
    FROM Laminating_h A
    INNER JOIN Laminating_d B ON B.NoLaminating = A.NoLaminating
    ...
    GROUP BY (CASE WHEN A.IdFisik IS NULL THEN G.Singkatan ELSE H.Singkatan END + ' ' + C.Jenis + ' ' + I.NamaGrade)
    ORDER BY Jenis
  `);
...
async function getLabels(jenis, tgl) {
  ...
  const result = await req.query(`
    ...
    INNER JOIN MstWarehouse G ON G.IdWarehouse = A.IdWarehouse
    FULL JOIN MstWarehouse H ON H.IdWarehouse = A.IdFisik
    WHERE (CASE WHEN A.IdFisik IS NULL THEN G.Singkatan ELSE H.Singkatan END + ' ' + C.Jenis + ' ' + I.NamaGrade) = @jenis
      AND A.DateCreate <= @tgl AND (A.DateUsage > @tgl OR A.DateUsage IS NULL)
    ORDER BY A.DateCreate ASC, A.NoLaminating
  `);
  return result.recordset;
}
...
async function getOldestDate() {
  ...
  const result = await pool.request().query(
    `SELECT MIN(DateCreate) AS oldestDate FROM Laminating_h`
  );
