...
async function getStock(tgl) {
  ...
  const result = await req.query(`
    ...
    FROM Moulding_h A
    INNER JOIN Moulding_d B ON B.NoMoulding = A.NoMoulding
...
async function getLabels(jenis, tgl) {
  ...
  const result = await req.query(`
    ...
    ORDER BY A.DateCreate ASC, A.NoMoulding
  `);
  return result.recordset;
}
...
async function getOldestDate() {
  ...
  const result = await pool.request().query(
    `SELECT MIN(DateCreate) AS oldestDate FROM Moulding_h`
  );
